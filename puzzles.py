...
class ChessPuzzle:
    ...
    def __init__(self, fen: str, solution: List[str], theme: str, difficulty: str, description: str = ""):
        """
...
class PuzzleTrainer:
    ...
    def _load_puzzles(self) -> List[ChessPuzzle]:
        ...
        return [
            ...
            ChessPuzzle(
                ...
                solution=["c4d5", "f6d5", "d1a4"],  # Take pawn, knight recaptures, check wins piece
                theme="Fork",
                difficulty="Easy",
    ...
    def get_random_puzzle(self, difficulty: Optional[str] = None) -> Optional[ChessPuzzle]:
    ...
    def solve_puzzle(self, puzzle: ChessPuzzle, stockfish_path: Optional[str] = None):
        ...
        while solution_index < len(puzzle.solution):
            ...
            if solution_index % 2 == 0:
                ...
                while True:
                    user_input = input("Enter your move (UCI format, or 'hint'/'solution'): ").strip().lower()
                    
                    if user_input == 'hint':
                        if stockfish_path and os.path.exists(stockfish_path):
                            self._show_hint(board, stockfish_path)
                        else:
                            print("💡 Hint: Look for forcing moves (checks, captures, threats)")
                        continue
                    ...
                    try:
                        ...
                        expected_move = chess.Move.from_uci(puzzle.solution[solution_index])
    ...
    def training_session(self, num_puzzles: int = 3, stockfish_path: Optional[str] = None):
        """
